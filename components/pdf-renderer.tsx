...
const PDFRenderer = ({ file }: PDFRendererProps) => {
  ...
  const CustomPageValidator = z.object({
    page: z
      .string()
      .refine((num) => Number(num) > 0 && Number(num) <= totalNumberOfPages!),
  });
      ...
      <div className="h-14 w-full border-b border-zinc-200 flex items-center justify-between px-2">
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            />
            <p className="text-zinc-700 text-sm space-x-1 flex items-center justify-center gap-1">
              <span>/</span>
              <span>
                {totalNumberOfPages ?? (
                  <Loader2 className="w-3 h-3 animate-spin flex" />
                )}
              </span>
            </p>
          </div>
          ...
          <Button
            ...
            onClick={() => {
              setCurrentPage((prev) =>
                prev + 1 > totalNumberOfPages! ? totalNumberOfPages! : prev + 1
              );
              setValue('page', String(currentPage + 1));
            }}
            variant="ghost"
